#!/usr/bin/env python3
"""
PostgreSQL → Neo4j ETL Pipeline - GEMINI INSTANCE
Extracts memories from wolf_logic (port 5436), builds graph in Neo4j (port 8688).
...
class MemoryGraphBuilder:
    ...
    def connect(self):
        ...
        print("[GEMINI] Connecting to PostgreSQL (port 5436)...")
        self.pg_conn = psycopg2.connect(**PG_CONFIG)

        print("[GEMINI] Connecting to Neo4j (port 8688)...")
        self.neo4j_driver = GraphDatabase.driver(
            NEO4J_CONFIG["uri"],
            auth=(NEO4J_CONFIG["user"], NEO4J_CONFIG["password"])
        )

        print("[GEMINI] Connected to both databases.\n")
    ...
    def setup_neo4j_schema(self):
        ...
        print("[GEMINI] Setting up Neo4j schema...")
        ...
        print("[GEMINI] Neo4j schema ready.\n")
    ...
    def process_memories(self, batch_size=1000):
        ...
        print("[GEMINI] Starting ETL pipeline...")
        print(f"[GEMINI] Batch size: {batch_size}\n")

        offset = 0
        batch_num = 1

        while True:
            print(f"[GEMINI] Processing batch #{batch_num} (offset: {offset})...")

            memories = self.fetch_memories_batch(offset, batch_size)

            if not memories:
                print("[GEMINI] No more memories to process.")
                break
            ...
            with self.neo4j_driver.session() as session:
                for memory in memories:
                    try:
                        ...
                        print(f"[GEMINI] Error processing memory {memory[0]}: {e}")
                        self.stats["errors"] += 1
    ...
    def build_similarity_edges(self, similarity_threshold=0.7, limit=100):
        ...
        """
        print(f"[GEMINI] Building similarity edges (threshold: {similarity_threshold})...")
        ...
        print(f"[GEMINI] Created {len(similarities)} similarity edges.\n")

    def print_stats(self):
        """Print final statistics"""
        print("=" * 60)
        print("[GEMINI] ETL PIPELINE COMPLETE")
        print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='[GEMINI] PostgreSQL to Neo4j ETL Pipeline')
