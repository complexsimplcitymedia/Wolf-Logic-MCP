#!/usr/bin/env python3
"""
Neo4j <-> Postgres Bridge - GEMINI INSTANCE
Connects Gemini's Neo4j graph to Gemini's Postgres tables
Port 5436 (PostgreSQL 18) -> Port 8688 (Neo4j Gemini)
"""
...
# Config - GEMINI'S ISOLATED ENVIRONMENT
NEO4J_URI = os.getenv("NEO4J_URI", "bolt://localhost:8688")
...
class Neo4jPostgresBridge:
    def __init__(self):
        ...
        print(f"[GEMINI] Connected to Neo4j: {NEO4J_URI}")
        print(f"[GEMINI] Connected to Postgres: {PG_HOST}:{PG_PORT}/{PG_DATABASE}")
    ...
    def sync_memories_to_neo4j(self):
        ...
        cursor.close()
        print(f"[GEMINI] Synced {count} memories to Neo4j")
        return count
    ...
    def sync_applications_to_neo4j(self):
        ...
        cursor.close()
        print(f"[GEMINI] Synced {count} applications to Neo4j")
        return count
    ...
    def query_neo4j_to_postgres(self, cypher_query, table_name, columns):
        ...
        if not records:
            print("[GEMINI] No results from Neo4j")
            return 0
        ...
        self.pg_conn.commit()
        cursor.close()
        print(f"[GEMINI] Inserted {count} records into {table_name}")
        return count
